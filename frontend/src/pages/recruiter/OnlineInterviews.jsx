// src/pages/recruiter/OnlineInterviews.jsx
...
export default function OnlineInterviews() {
  ...
  const fetchInterviews = useCallback(async () => {
    try {
      ...
      toast.error(err.response?.data?.detail || 'Failed to load online interviews.');
      setInterviews([]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="min-h-[calc(100vh-72px)] bg-linear-to-br from-slate-50 via-white to-indigo-50 px-4 py-5 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl rounded-4xl border border-red-200 bg-white p-8 text-center shadow-xl shadow-slate-200/60">
          ...
          <p className="mt-2 text-sm font-semibold text-slate-500">
            Only recruiters and admins can manage online interviews.
          </p>
            ...
            <div className="relative z-10 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-300">
                  Manage video interviews, meeting links, notes, feedback, and interview status.
                </p>
              </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h2 className="text-xl font-black text-slate-950">
                Scheduled Online Interviews
              </h2>

              <p className="mt-1 text-sm font-semibold text-slate-500">
                Click Join Meeting to open Zoom, Google Meet, or Teams link.
              </p>
            </div>
          ...
          {loading ? (
            <div className="flex min-h-96 items-center justify-center rounded-3xl bg-slate-50">
              <div className="text-center">
                ...
                <p className="mt-2 text-sm font-semibold text-slate-500">
                  Fetching online interview schedule...
                </p>
              </div>
            </div>
          ) : interviews.length === 0 ? (
            <div className="flex min-h-96 items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
              <div>
                ...
                <h3 className="mt-4 text-lg font-black text-slate-900">
                  No online interviews scheduled
                </h3>

                <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
                  Schedule interviews from the Interview Management page first.
                </p>
...
function OnlineInterviewCard({
  ...
  const safeMeetingLink = getSafeMeetingLink(interview.location);
  ...
  return (
    <div>
      ...
      <select
        ...
        value={value}
        onChange={onChange}
        className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-bold text-slate-700 outline-none transition focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10"
      >
        {children}
      </select>
    </div>
